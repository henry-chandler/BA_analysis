...
Inputs:
    Filtered combined event dataset from 03_combine_and_filter_events.py:
        <RAW_INF_DIR>/BAEI_events_2022-2026.npy
    Seismic detection results (if available):
        <RAW_SEISMIC_DIR>/seismic_detections.npy
    ...
    Raw daily seismic miniSEED files:
        <RAW_SEISMIC_DIR>/BAT/BAT..BHZ.YYYY-MM-DDTHH-MM-SS_24h_100Hz.ms
        <RAW_SEISMIC_DIR>/BAE/BAE..BHZ.YYYY-MM-DDTHH-MM-SS_24h_100Hz.ms

Outputs:
    Annotated event dataset:
        <RAW_INF_DIR>/BA_event_quality_2022-2026.npy
    Optional high-resolution event figures (300 dpi):
        <FIGURE_OUT_DIR>/event_NNNNN.png
...
# EVENT_FILE should be the location of the resulting same file from script 03_combine_and_filter
RAW_INF_DIR = ''
RAW_SEISMIC_DIR = ''
EVENT_FILE = ('')
OUTPUT_NPY = ('')
SEISMIC_STATIONS = ['BAT', 'BAE']

# Output directory for saved figures
FIGURE_OUT_DIR = ''

# Events to save when "PRINT SELECTED" is clicked — edit this list as needed
BATCH_SAVE_EVENTS = [10, 23, 29, 301]
...
def build_figure(i, event_times, event_dur, raw_inf_dir, raw_seismic_dir,
                 seismic_station):
    """
    Build and return a matplotlib Figure for event i, showing waveform and
    spectrogram panels for infrasound (BAEI) and any available seismic stations.
    ...
    event_dur : np.ndarray
        Event durations [s].
    raw_inf_dir : str
        Base directory for infrasound data files.
    raw_seismic_dir : str
        Base directory for seismic data files.
    seismic_station : list of str
        Seismic station codes to check (e.g. ['BAT', 'BAE']).
    ...
    inf_file = os.path.join(raw_inf_dir, f'BA_{event_times[i].year:04d}',
                            f'BAEI..HDF.{date_str}')
    bat_file = os.path.join(raw_seismic_dir, seismic_station[0],
                            f'{seismic_station[0]}..BHZ.{date_str}')
    bae_file = os.path.join(raw_seismic_dir, seismic_station[1],
                            f'{seismic_station[1]}..BHZ.{date_str}')

    sources = [('Infrasound (BAEI HDF)', inf_file)]
    if os.path.exists(bat_file):
        sources.append((f'Seismic {seismic_station[0]} (BHZ)', bat_file))
    if os.path.exists(bae_file):
        sources.append((f'Seismic {seismic_station[1]} (BHZ)', bae_file))
    ...
    for label, fpath in sources[:]:
        try:
            ...
            sources = [(l, p) for l, p in sources if l != label]
    ...
    y_labels = {'Infrasound (BAEI HDF)': 'Pressure (Pa)'}
    spec_ylims = {'Infrasound (BAEI HDF)': 30}
    for label, _ in sources[1:]:
        y_labels[label]   = 'Velocity (m/s)'
        spec_ylims[label] = 20
    ...
    for row, (label, _) in enumerate(sources):
        ...
        ax_raw.set_ylabel(y_labels.get(label, 'Amplitude'), fontsize=9)
        ax_raw.xaxis_date()
        ax_raw.xaxis.set_major_formatter(mdates.DateFormatter('%H:%M:%S'))

        # Infrasound onset/offset markers on the top row only
        if row == 0:
            ax_raw.axvline(inf_onset, color='red', linestyle='--', linewidth=1.5)
            ax_raw.axvline(inf_offset, color='blue', linestyle='--', linewidth=1.5)

        plot_spectrogram(ax_spec, tr, SPEC_WLEN, SPEC_OVERLAP,
                         ylim=spec_ylims.get(label, 20))
...
class EventReviewer:
    def __init__(self, root):
        ...
        # Load event data
        df = np.load(EVENT_FILE, allow_pickle=True).item()
        ...
        if os.path.exists(OUTPUT_NPY):
            saved = np.load(OUTPUT_NPY, allow_pickle=True).item()
    ...
    def _build_ui(self):
        ...
        self.entry_seismic_recording = field(
            form, 'SEISMIC RECORDING', 'e.g. b = both / t = BAT / e = BAE / n = neither')
        self.entry_quality = field(
            form, 'QUALITY', 'e.g. h = high / m = medium / p = poor')
        ...
        batch_hint = ', '.join(str(n) for n in BATCH_SAVE_EVENTS)
    ...
    def _save_current(self):
        ...
        self.status_var.set(f'Saved event {idx} → {OUTPUT_NPY}')
        self._refresh_table()
    ...
    def _save_all_outputs(self):
        ...
        np.save(OUTPUT_NPY, df_to_dict(full))
    ...
    def _render_and_save(self, idx, silent=False):
        ...
        try:
            ...
            fig = build_figure(idx, self.event_times, self.event_dur,
                               RAW_INF_DIR, RAW_SEISMIC_DIR,
                               SEISMIC_STATIONS)
            out_path = os.path.join(FIGURE_OUT_DIR, f'event_{idx:05d}.png')
