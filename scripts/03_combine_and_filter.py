...
#   likely called BAEI..HDF.YYYY-MM-DD_to_YYY-MM-DD_event_id
EVENT_FILES = {
    2022: os.path.join(INPUT_INFRASOUND_DIR, ''),
    2023: os.path.join(INPUT_INFRASOUND_DIR, ''),
    2024: os.path.join(INPUT_INFRASOUND_DIR, ''),
    2025: os.path.join(INPUT_INFRASOUND_DIR, '')
}
...
frames = []
for year, filepath in EVENT_FILES.items():
    df = load_year(filepath, year)
    if df is not None:
        frames.append(df)

if len(frames) == 0:
    raise FileNotFoundError('No event files were found. Check EVENT_FILES paths.')
...
years      = sorted(EVENT_FILES.keys())
label      = OUTPUT_LABEL or f'BAEI_events_filtered_{years[0]}-{years[-1]}'
